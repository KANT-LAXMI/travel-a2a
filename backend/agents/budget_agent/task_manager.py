...
class BudgetTaskManager(InMemoryTaskManager):
    def __init__(self):
        super().__init__()
        self.agent = BudgetAgent()

    async def on_send_task(self, request: SendTaskRequest) -> SendTaskResponse:
        logger.info(f"BudgetTaskManager received task: {request.params.id}")
        ...
        logger.info(f"BudgetTaskManager completed task: {request.params.id}")
