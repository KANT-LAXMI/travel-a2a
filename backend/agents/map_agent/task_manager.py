...
class MapTaskManager(InMemoryTaskManager):
    def __init__(self):
        super().__init__()
        self.agent = MapAgent()

    async def on_send_task(self, request: SendTaskRequest) -> SendTaskResponse:
        logger.info(f"MapTaskManager received task: {request.params.id}")
        
        # Upsert task
        task = await self.upsert_task(request.params)
        
        # Get itinerary text from user query
        ...
        logger.info(f"MapTaskManager completed task: {request.params.id}")
